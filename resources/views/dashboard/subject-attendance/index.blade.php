                ...
                @foreach ($today_schedules as $today_schedule)
                    <div class="glass-card rounded-lg p-4">
                        ...
                        <div class="flex flex-col space-y-2">
                            <a class="{{ $today_schedule->is_subject_attended_today ? 'bg-blue-500 hover:bg-blue-600' : 'bg-purple-600 hover:bg-purple-700' }} flex items-center justify-center rounded-lg px-4 py-2 text-sm font-medium text-white" href="{{ route('subject-attendance.create', $today_schedule->id) }}">
                                <x-lucide-list-checks class="mr-2 h-4 w-4" />
                                {{ $today_schedule->is_subject_attended_today ? 'Perbarui Absen' : 'Mulai Absen' }}
                            </a>
                        </div>
                    </div>
                @endforeach
                        ...
                        <tr class="hover:bg-white/5">
                            ...
                            <td class="whitespace-nowrap px-4 py-4 text-right">
                                <div class="flex justify-end space-x-2">
                                    <a class="{{ $today_schedule->is_subject_attended_today ? 'bg-blue-600/20 hover:bg-blue-700/20 text-blue-300' : 'bg-purple-600/20 hover:bg-purple-700/20 text-purple-300' }} flex items-center justify-center rounded-lg px-4 py-2 text-sm font-medium" href="{{ route('subject-attendance.create', $today_schedule->id) }}">
                                        <x-lucide-list-checks class="mr-2 h-4 w-4" />
                                        {{ $today_schedule->is_subject_attended_today ? 'Perbarui Absen' : 'Mulai Absen' }}
                                    </a>
                                </div>
