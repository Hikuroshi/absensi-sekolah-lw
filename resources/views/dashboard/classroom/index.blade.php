<x-layout.app :$title>
    ...
    <div class="glass-card rounded-xl p-6">
        <div class="mb-6 flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
            ...
            <a class="flex w-full items-center justify-center rounded-lg bg-purple-600 px-4 py-2 text-sm font-medium text-white hover:bg-purple-700 sm:w-auto" href="{{ route('classroom.create') }}">
                ...
                @foreach ($classrooms as $classroom)
                    <div class="glass-card flex items-center justify-between rounded-lg p-4">
                        <div class="text-sm">
                            ...
                            <div class="mb-2 font-medium">{{ $classroom->name }}</div>

                            <div class="text-xs text-white/60">Tahun Ajaran</div>
                            <div>{{ $classroom->year }}</div>
                        </div>
                        <div class="flex flex-col space-y-2">
                            <a class="rounded-lg p-1 text-blue-400 hover:bg-white/10" href="{{ route('classroom.show', $classroom->id) }}">
                                <x-lucide-eye class="h-4 w-4" />
                            </a>
                            <a class="rounded-lg p-1 text-blue-400 hover:bg-white/10" href="{{ route('classroom.edit', $classroom->id) }}">
            ...
            <table class="hidden w-full sm:table">
                <thead>
                    <tr class="border-b border-white/10 text-left text-sm text-white/80">
                        ...
                        <th class="whitespace-nowrap px-4 py-3">Tahun Ajaran</th>
                ...
                <tbody class="divide-y divide-white/10">
                    @foreach ($classrooms as $classroom)
                        <tr class="hover:bg-white/5">
                            ...
                            <td class="whitespace-nowrap px-4 py-4 font-medium">{{ $classroom->name }}</td>
                            <td class="whitespace-nowrap px-4 py-4">{{ $classroom->year }}</td>
                            <td class="whitespace-nowrap px-4 py-4 text-right">
                                <div class="flex justify-end space-x-2">
                                    <a class="rounded-lg p-2 text-blue-400 hover:bg-white/10" href="{{ route('classroom.show', $classroom->id) }}">
                                        <x-lucide-eye class="h-4 w-4" />
                                    </a>
                                    <a class="rounded-lg p-2 text-blue-400 hover:bg-white/10" href="{{ route('classroom.edit', $classroom->id) }}">
                                        <x-lucide-edit class="h-4 w-4" />
                                    </a>
                    ...
                    @if ($classrooms->isEmpty())
                        <tr>
                            <td class="py-4 text-center text-white/60" colspan="4">Tidak ada kelas ditemukan</td>
                        </tr>
                    @endif
                </tbody>
            </table>
    ...
    <x-confirm-modal id="delete-classroom-modal" />
</x-layout.app>
