<x-layout.app :$title>
    ...
    <form action="{{ $old_attendance?->isNotEmpty() ? route('student-attendance.update', [$old_summary_id, $session_type]) : route('student-attendance.store', $session_type) }}" method="POST">
        @csrf

        <div class="glass-card rounded-xl p-6">
            <div class="mb-6 flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
                <div class="w-full">
                    <h2 class="text-xl font-semibold">Absensi {{ $session_type->label() }}: {{ $classroom->name }} - {{ now()->translatedFormat('l') }}</h2>
                    <p class="mb-6 text-sm text-white/60">
                        Absensi <span class="font-bold">{{ $session_type->label() }} </span>
                        di <span class="font-bold">{{ $classroom->name }} </span>
                        pada hari <span class="font-bold">{{ now()->translatedFormat('l') }} </span>
                            ...
                            <div class="text-sm">
                                ...
                                <div class="mb-2 w-full">
                                    <input
                                        ...
                                        type="text"
                                        value="{{ old('attendances[' . $member->id . '][note]', $old_attendance[$member->id]->note ?? '') }}"
                                ...
                                <div class="mb-2 w-full">
                                    <select class="cool-select w-full" id="status_{{ $member->id }}" name="attendances[{{ $member->id }}][status]" required>
                                        ...
                                        @foreach ($statuss as $status)
                                            <option value="{{ $status->value }}" @selected(old('attendances[' . $member->id . '][status]', $old_attendance[$member->id]->status->value ?? '') === $status->value)>
                                                {{ $status->label }}
                                            </option>
                                        @endforeach
                                    </select>
